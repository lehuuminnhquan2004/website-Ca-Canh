...
if (isset($_GET['delete'])) {
    ...
    $result = mysqli_query($conn, "SELECT thumbnail FROM fishs WHERE id = $id");
    $row = mysqli_fetch_assoc($result);

if ($row && !empty($row['thumbnail'])) {
    $thumb = $row['thumbnail'];
    // Chuẩn hóa về thư mục uploads gốc
    ...
    mysqli_query($conn, "DELETE FROM fishs WHERE id = $id");
...
/* -----------------------------
   Đếm tổng số cá
----------------------------- */
$countSql = "SELECT COUNT(*) AS total FROM fishs WHERE 1";
...
/* -----------------------------
   SORT
----------------------------- */
$orderBy = "f.created_at DESC"; // default newest
...
if ($selectedCategoryId > 0) {
    $sql .= " AND f.category_id = $selectedCategoryId";
}

if (!empty($searchKeyword)) {
    $searchEscaped = mysqli_real_escape_string($conn, $searchKeyword);
    $sql .= " AND f.name LIKE '%$searchEscaped%'";
}
if ($filterNoImage) {
    $sql .= " AND (f.thumbnail IS NULL OR f.thumbnail = '')";
}
if ($filterStatus === 0 || $filterStatus === 1) {
    $sql .= " AND f.status = $filterStatus";
}
...
<div class="container">
    ...
    <div style="margin-bottom: 10px;">
        <a class="btn" href="./fish-form.php<?= $returnQuery ? ('?return=' . urlencode($returnQuery)) : '' ?>">+ Thêm cá cảnh</a>
    </div>
        ...
        <table class="table">
            ...
            <?php while ($row = mysqli_fetch_assoc($res)): ?>
                <tr>
                    ...
                    <td>
                        <a class="btn small" href="./fish-form.php?id=<?= $row['id'] ?><?= $returnQuery ? ('&return=' . urlencode($returnQuery)) : '' ?>">Sửa</a>
                        <a class="btn small danger" 
                           href="./fishs.php?delete=<?= $row['id'] ?>"
